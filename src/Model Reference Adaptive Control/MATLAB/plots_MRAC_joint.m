...
ctrl_mode   = 'fixed';   % Caso simulado: 'adaptive' | 'fixed'
...
switch mode
    case 'sim'
        csv_file   = fullfile(pkg_dir, 'data', 'lab7', 'sim', 'act1', ...
                              sprintf('gz_mrac_joint_%s_%d.csv', ctrl_mode, test_num));
        output_dir = fullfile(pkg_dir, 'plots', 'lab7', 'sim', 'act1', ...
...
fprintf('%s\n', repmat('-', 1, 88));
fprintf(' Parametros estimados finales:  mL=%.4f kg   b=[%.4f %.4f %.4f %.4f] N·m·s/rad\n', ...
        mL_hat(end), b_hat(end,1), b_hat(end,2), b_hat(end,3), b_hat(end,4));
fprintf('%s\n\n', repmat('═', 1, 88));
...
% Masa de carga estimada
nexttile(tl6);
plot(t, mL_hat, '-', 'Color', color_meas, 'LineWidth', lw); hold on;
yline(0, ':', 'LineWidth', 0.9, 'Color', [0.5 0.5 0.5]);
xlabel('Tiempo [s]', 'FontSize', fs);
ylabel('$\hat{m}_L$ [kg]', 'Interpreter', 'latex', 'FontSize', fs);
title('Masa de carga estimada', 'FontSize', fs_title - 2);
grid on; box on; set(gca, 'FontSize', fs); xlim(xlims);

% Fricciones viscosas estimadas
nexttile(tl6);
h_b = gobjects(1, 4);
for i = 1:4
...
title('Fricciones viscosas estimadas', 'FontSize', fs_title - 2);
