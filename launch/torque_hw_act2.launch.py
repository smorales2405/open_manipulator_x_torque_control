#!/usr/bin/env python3
#
# torque_hw_act2.launch.py
# Lanza el control IO cartesiano sobre hardware real.
# Usa Dynamixel SDK directo — NO compatible con hardware.launch.py.
#
# Uso:
#   ros2 launch open_manipulator_x_torque_control torque_hw_act2.launch.py
#   ros2 launch ... torque_hw_act2.launch.py gain_scale:=0.5 log_id:=2
#
# Argumentos sobreescribibles desde CLI:
#   gain_scale      — escala de ganancias cartesianas (1.0 = base)
#   deadzone_ticks  — compensación zona muerta [ticks]
#   viscous_comp    — compensación viscosa
#   duration_s      — duración [s] (debe ser > 3.0 por la fase de transición)
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('duration_s',     default_value='25.0',
                              description='Debe ser mayor que T_TRANS=3.0 s'),
        DeclareLaunchArgument('log_id',         default_value='1',
                              description='Identificador del CSV hw_io_data_<log_id>.csv'),

        Node(
            package=PKG,
            executable='hw_io_control_node',
            name='hw_io_control_node',
            output='screen',
            parameters=[
                params_file,
                {
